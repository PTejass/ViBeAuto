...
let rateLimitCounter = 0; // New: Global counter for rate limits
...
chrome.storage.local.get(['autoEnabled', 'vcamEnabled', 'vcamSource', 'vidSpeed', 'speedEnabled'], (res) => {
    ...
    // Constant enforcement loop (runs in all contexts)
    setInterval(() => {
...
async function checkPageState() {
    ...
    const submitBtn = clickable.find(el => {
        const t = getText(el);
        return (t.includes('submit') || t.includes('check') || t.includes('finish')) && !t.includes('flag');
    });
    
    const nextLessonBtn = clickable.find(el => {
        const t = getText(el);
        return (t.includes('next lesson') || t.includes('continue')) && t.length < 35;
    });
    ...
    if (bodyText.includes('failed to stop video') || bodyText.includes('unable to save progress')) {
        const errBtn = clickable.find(el => getText(el) === 'continue');
    ...
    // SCENARIO 0: Quiz Success
    if (bodyText.includes('quiz completed') || bodyText.includes('passed!') || bodyText.includes('you scored')) {
        if (nextLessonBtn && !nextLessonBtn.dataset.vibePending) {
            nextLessonBtn.dataset.vibePending = "true";
            setTimeout(() => simulateClick(nextLessonBtn), 1000);
        }
        return;
    }
    ...
    if (options.length > 0) {
        ...
        if (isAnswered) {
             const btn = submitBtn || clickable.find(el => getText(el) === 'next') || nextLessonBtn;
             if (btn && !btn.dataset.vibePending) {
                 btn.dataset.vibePending = "true";
                 setTimeout(() => simulateClick(btn), 500);
             }
             return; 
        }
        ...
        const actionBtn = submitBtn || clickable.find(el => getText(el) === 'next') || clickable.find(el => getText(el).includes('next'));
        
        const payload = `[COURSE CONTEXT]\nTitle: ${document.title}\n\n[QUIZ CONTENT]\n${main.innerText}`;
        const result = await solveQuiz(payload, optsText, options, actionBtn);

        // --- Post-Quiz Delay Logic (New: Maximum Speed vs Resilient Delay) ---
        if (result.rateLimit) {
            rateLimitCounter++;
            if (rateLimitCounter > 20) {
                window.alert("️ViBe Auto: Rate limit hit 20 times. Stopping automation.");
            ...
            rateLimitCounter = 0; // Reset on success
            // Maximum speed: No artificial delay after successful solve
            setTimeout(() => { isProcessingQuiz = false; }, 300); // 300ms small buffer to prevent double-firing
        }
        return;
    }

    // SCENARIO B: Video Progress
    const video = document.querySelector('video');
    if (video && (video.paused || video.ended) && nextLessonBtn && options.length === 0) {
        if (!nextLessonBtn.dataset.vibePending) {
            nextLessonBtn.dataset.vibePending = "true";
            setTimeout(() => { simulateClick(nextLessonBtn); nextLessonBtn.dataset.vibePending = ""; }, 1000);
        }
    }
}
...
function enforceVideoFeatures(applySpeed) {
    ...
    document.querySelectorAll('video').forEach(v => {
        ...
        if (v.paused && !v.ended && v.readyState >= 2) {
            v.play().catch(e => {
                if (e.name === "NotAllowedError" || e.name === "NotSupportedError") { v.muted = true; v.play().catch(() => {}); }
            });
        }
    });
}
...
async function solveQuiz(payload, optionsText, optionEls, submitButton) {
    return new Promise((res) => {
        chrome.runtime.sendMessage({ action: "solveQuiz", questionData: { question: payload, options: optionsText } }, (resp) => {
            if (!resp || resp.error) {
                if (resp && resp.error === "RATE_LIMIT_HIT") {
                    res({ success: false, rateLimit: true }); 
                } else {
            ...
            if (matched) {
                ...
                setTimeout(() => { 
                    ...
                    res({ success: true, rateLimit: false }); 
                }, 1500); 
